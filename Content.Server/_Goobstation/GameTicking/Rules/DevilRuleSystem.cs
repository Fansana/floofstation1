...
using System.Text;
using Content.Server._Goobstation.Devil;
...
public sealed class DevilRuleSystem : GameRuleSystem<DevilRuleComponent>
{
    ...
    private readonly EntProtoId _devilMindRole = "DevilRoleComponent";
    ...
    private bool MakeDevil(EntityUid target, DevilRuleComponent rule)
    {
        ...
        _role.MindAddRole(mindId, new DevilRoleComponent(), mind, true);
        ...
        if (_role.MindHasRole<DevilRoleComponent>(mindId))
            AddComp(mindId, new RoleBriefingComponent { Briefing = briefing }, overwrite: true);
